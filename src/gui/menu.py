...
Все компоненты меню в одном файле
"""
import arcade
...
class MenuView(arcade.View):
    ...
    def __init__(self, on_start_game: Callable):
        ...
        self.characters = {
            "warrior": {"name": "Воин", "damage": 15, "health": 100, "speed": 1.0},
            "mage": {"name": "Маг", "damage": 25, "health": 70, "speed": 1.1},
            "archer": {"name": "Лучник", "damage": 20, "health": 80, "speed": 1.2}
        }
    ...
    def load_sounds(self):
        ...
        try:
            # Пытаемся найти звуки относительно текущей директории
            sound_path = "gui/assets/sounds/"
            if os.path.exists(sound_path):
                self.click_sound = arcade.load_sound(f"{sound_path}click.wav")
                self.select_sound = arcade.load_sound(f"{sound_path}select.wav")
                self.death_sound = arcade.load_sound(f"{sound_path}death.wav")
            else:
                # Пробуем найти в родительской директории
            ...
            print(f"Звуковые файлы не найдены: {e}. Продолжаем без звуков")
    ...
    def create_buttons(self):
        ...
        for text, callback, x, y in buttons:
            button = arcade.gui.UIFlatButton(
                ...
                width=BUTTON_WIDTH,
                height=BUTTON_HEIGHT,
                style={"font_size": 20, "bg_color": arcade.color.BLUE_GRAY}
            )
...
__all__ = ['MenuView', 'CharacterSelectView', 'SettingsView', 'GameSettings']