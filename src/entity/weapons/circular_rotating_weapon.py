...
class CircularRotatingWeapon(BaseWeapon):
    def __init__(self, path_or_texture: str | Path | bytes | Texture = "assets/weapons/axe.png", damage: float = 3, radius: float = 100, speed: float = 1) -> None:
        super().__init__(path_or_texture, damage)
        self.set_stat("radius", radius/100)
    ...
    def update(self, delta_time: float, enemies_list: SpriteList[BaseEnemy], player: Player):
        ...
        self.center_x = player.center_x + (self.get_stat("radius")*100*math.cos(self.radians))
        self.center_y = player.center_y + (self.get_stat("radius")*100*math.sin(self.radians))
