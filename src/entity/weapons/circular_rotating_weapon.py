...
class CircularRotatingWeapon(BaseWeapon):
    ...
    def update(self, delta_time: float, enemies_list: SpriteList[BaseEnemy], player: Player):
        self.scale = self.get_stat("scale")/100
        self.angle += math.pi*delta_time*self.get_stat("speed")/10
        self.center_x = player.center_x + (self.get_stat("radius")*100*math.cos(self.angle))
        self.center_y = player.center_y + (self.get_stat("radius")*100*math.sin(self.angle))
